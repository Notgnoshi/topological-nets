...
\begin{enumerate}
    ...
    \item The collection $\mathcal A$ of subsets of $S$ that is closed under finite intersections, partially ordered by reverse inclusion. That is, $A \preceq B$ if $A \supset B$.
          \begin{proof}
              Let $\mathcal A$ be a collection of subsets of a set $S$ that is closed under finite intersections. That is, if $A_1, \dots, A_n \in \mathcal A$, then $\bigcap_{i = 1}^n A_i \in \mathcal A$. Suppose $A, B, C \in \mathcal A$.\todo{I don't see how to use the hypothesis that $\mathcal A$ is closed under finite intersections.}
